...
read -p "jupyter port (9200): " port
test -z "$port" && port="9200"
...
read -p "container name (scala-ws1): " container
test -z "$container" && container="scala-ws1"
...
docker run --detach --publish=$port:$port --name="${container}" \
    ...
    jupyter-scala:latest python3 /root/configs/jupyter.py "$port" "$password"
