...
read -p "container name (jupyter-ws1): " container
test -z "$container" && container="jupyter-ws1"
...
docker run --detach --publish=$port:$port --name="${container}" \
    ...
    jupyter:latest python3 /root/configs/jupyter.py "$port" "$password"
